"""
AI Learning Companion + Speech-to-Text Backend
===============================================

A FastAPI service that provides:

1. Google Gemini powered AI chat
2. Chat sessions and personas
...
8. Local moderation
9. Speech-to-Text using Faster-Whisper

Run:
    python -m pip install -r requirements.txt
    python -m pip install python-multipart faster-whisper
...
if not _gemini_api_key:
    print(
        ...
        "Set it in the .env file before making Gemini requests."
    )

client = genai.Client(
    api_key=_gemini_api_key or "missing-api-key"
)
...
#
# "base" is a good starting point for this project because:
...
# Lazy loading:
# We don't download/load Whisper when the server starts.
# It loads only when /api/transcribe is called.
_whisper_model = None


def get_whisper_model():
    """
    Load Faster-Whisper only when it is needed.

    This prevents the FastAPI server from downloading/loading
    the Whisper model during startup.
    """
...
app = FastAPI(
    title="AI Learning Companion + Speech-to-Text API",
    description=(
        "AI Learning Companion backend with Gemini chat "
        "and Faster-Whisper speech-to-text."
    ),
    version="1.0.0",
)
...
class StudyPlan(BaseModel):
    """
    Validates structured JSON generated by Gemini.
    """
...
TOOL_SCHEMAS = [
...

    {
        ...
        "description": (
            "Search a mock study-topic database "
            "for a short reference note on a subject."
        ),
        "parameters_json_schema": {
            "type": "object",
            "properties": {
...
MOCK_DB = {
    ...
    "photosynthesis":
        ...
        "into chemical energy (glucose) using CO2 and water, "
        "releasing oxygen.",

    "newton's laws":
        "Three laws describing motion: "
        "(1) inertia, (2) F=ma, "
        "(3) equal & opposite reactions.",

    "french revolution":
        "1789-1799 period of radical political/social "
        "upheaval in France that ended the monarchy.",

    "big o notation":
        "Describes the upper-bound growth rate of an algorithm's "
        "time or space requirements as input size grows.",
}
...
def execute_tool(
    ...
    try:
        ...
        if name == "calculate":
            ...
            if (
                not expr
                or not set(expr).issubset(allowed)
            ):
                return json.dumps({
                    "error":
...
def root():
    ...
    return {
        "status": "ok",
        "service":
            "AI Learning Companion + Speech-to-Text API",
        "speech_to_text":
            "Faster-Whisper",
    }
...
async def transcribe_audio(
    ...
    try:
        ...
        # Save temporarily because Faster-Whisper can process
        # a local audio file path directly.
        ...
        print(
            f"Transcribing audio: {original_filename}"
        )
    ...
    try:
        ...
        if req.use_tools:
            ...
            if (
                ...
                and
                "json"
                not in system_instruction.lower()
            ):
...
def structured_json_prompt(
    req: ProductionPromptRequest,
):

    template = PRODUCTION_PROMPTS[
        "structured_json"
    ]
    ...
    raise HTTPException(
        status_code=500,
        detail=(
            "Model output did not conform "
            "to the JSON schema after "
...
def production_prompt(
    ...
    if (
        prompt_type not in PRODUCTION_PROMPTS
        or prompt_type == "structured_json"
    ):

        raise HTTPException(
            status_code=404,
            detail=(
                f"Unknown prompt type "
                f"'{prompt_type}'."
            ),
        )
